...
import { getFirestore, collection, getDocs, addDoc, updateDoc, deleteDoc, doc, getDoc, setDoc, onSnapshot, query, orderBy, } from 'firebase/firestore';
...
localDB[key] = data;
console.log(`📝 Updated localDB.${key}:`, Array.isArray(data) ? data.length : typeof data);
}
...
const notifString = JSON.stringify(localDB.notifications);
const dbString = JSON.stringify(localDB);

localStorage.setItem('ishiaya_notifications', notifString);
localStorage.setItem('ishiayaDB', dbString);
...
await saveToLocalStorage('notifications', localDB.notifications);
...
// Fallback to localStorage when not using Firebase
loadFromLocalStorage();
console.log('📊 Getting notifications from localStorage:', localDB.notifications.length);
...
// Find this function near line 995-1010 and replace it:
...
export const notifyScheduleChange = async (employeeId, employeeName, changeDetails) => {
    try {
        ...
        // Changed from addNotification to createNotification
